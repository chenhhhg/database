...
public class RegionDataHandler extends AbstractTableDataHandler<Region> {
    ...
    public List<Region> cleanData(List<List<String>> records) {
        ...
        for (List<String> record : records) {
            if (validateRecord(record)) {
                ...
                invalidCount++;
                log.info("无效的Region记录被过滤: {}", record);
            }
        }
    ...
    public boolean validateRecord(List<String> record) {
        ...
        if (record == null || record.size() != fieldNames.size()) {
            log.info("Region记录字段数量不匹配，期望: {}, 实际: {}", 
                fieldNames.size(), record != null ? record.size() : 0);
            return false;
        }
        
        // 验证主键字段（r_regionkey）不能为空
        String regionkey = record.get(0); // r_regionkey是第一个字段
        if (regionkey == null || regionkey.trim().isEmpty()) {
            log.info("Region主键字段r_regionkey为空");
            return false;
        }
        
        // 特殊清洗：r_regionkey数值范围检查（>=0）
        try {
            Integer regionkeyValue = Integer.valueOf(regionkey.trim());
            if (regionkeyValue < 0) {
                log.info("Region主键字段r_regionkey小于0: {}", regionkeyValue);
                return false;
            }
        } catch (NumberFormatException e) {
            log.info("Region主键字段r_regionkey格式无效: {}", regionkey);
            return false;
        }
        ...
        if (maxLength != null && name.trim().length() > maxLength) {
            log.info("Region区域名称字段r_name长度超限: 当前={}, 最大={}", name.trim().length(), maxLength);
            return false;
        }
    ...
    public Region convertToEntity(List<String> record) {
        ...
        try {
            ...
            log.info("成功转换Region记录: regionkey={}, name={}", region.getRRegionkey(), region.getRName());
            return region;
    ...
    protected Long getMaxPrimaryKey() {
        try {
            ...
            if (maxRegion != null && maxRegion.getRRegionkey() != null) {
                ...
                log.info("Region表最大主键值: {}", maxKey);
                return maxKey;
            } else {
                log.info("Region表为空，返回主键值: 0");
                return 0L;
            }
        } catch (Exception e) {
    ...
    protected void setEntityPrimaryKey(Region entity, Long primaryKeyValue) {
        if (entity != null) {
            ...
            log.info("设置Region主键: regionkey={}", primaryKeyValue);
        }
    }
    ...
    private int insertOneByOne(List<Region> entities) {
        ...
        for (Region region : entities) {
            try {
                ...
                failCount++;
                log.info("插入单条Region记录失败: regionkey={}, 错误: {}", 
